...
void fp::algorithm::Solve(std::shared_ptr<fp::landbasedrobot> robot)
{
    ...
    while(!CheckGoal(x,y)) //Iterates till the goal is converged
    {
        ...
        if ( y-1 >= 0 && (!VisitedNodes[x][y-1]) && (!Maze.WallArray[SetCoordinates(x, y)][SetCoordinates(x, y - 1)])){
        ...
        else if ((x+1 >= 0) && (x+1 < 16) && (!VisitedNodes[x+1][y]) && (!Maze.WallArray [SetCoordinates(x, y)][SetCoordinates(x + 1, y)])){
            x = robot->get_x(); //Getting the current robot x coordinate
            y = robot->get_y(); //Getting the current robot y coordinate
        ...
        else if ( y+1 < 16 && (!VisitedNodes[x][y+1])  &&(!Maze.WallArray [SetCoordinates(x, y)][SetCoordinates(x, y + 1)]))
        {
            x = robot->get_x(); //Getting the current robot x coordinate
            y = robot->get_y(); //Getting the current robot y coordinate
        ...
        else if ((x-1 >= 0) && (!VisitedNodes[x-1][y]) && (!Maze.WallArray [SetCoordinates(x, y)][SetCoordinates(x - 1, y)])){
            x = robot->get_x(); //Getting the current robot x coordinate
            y = robot->get_y(); //Getting the current robot y coordinate
...
std::array<int, 2> fp::algorithm::GetCoordinates(int coordinates) {
    point[1] = coordinates/16; //x coordinate of the robot
    point[0] = coordinates%16; //y coordinate of the robot
    return point;
}
...
void fp::algorithm::ClearPath(std::stack<int> s) {
    ...
    while (!s.empty())
    {
        ...
        std::array<int, 2> point;
        point = GetCoordinates(x);
        if (!CheckGoal(point[0], point[1]) && !(point[0] == 0 && point[1] ==0))
        {
